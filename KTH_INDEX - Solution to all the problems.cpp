...
int main()
{
	...
	while(q--)
	{
	    ...
	    {
	         int len=ump[p].size();
	       auto it=lower_bound(ump[p].begin(),ump[p].end(),l);
	       if(it==ump[p].end())
	       {
	           auto ix=upper_bound(ump[p].begin(),ump[p].end(),l);
	           if(ix==ump[p].end())
	           cout<<"-1\n";
	           else
	           {
	              
	               int pos=ix-ump[p].begin();
	               if((pos+k-1 < len) and ump[p].at(pos+k-1)<=r)
	               {
	                   cout<<ump[p].at(pos+k-1)<<"\n";
	               }
	               else
	               cout<<"-1\n";
	           }
