...
int main() {
    ...
    test{
       ...
       if(str.find('1')==string::npos or str.find('2')==string::npos or str.find('3')==string::npos)
       {
           cout<<"0\n";
       }
       else
       {
       int len=1,end=-1,ans=INT_MAX;
       bool flag=false;
       int n=str.size();
       ust.insert(str.at(0));
       for(int i=1;i<n;i++)
           ...
           else
           {
               for(int j=i;j<n;j++)
               {
                   ...
                   if(ust.size()==3)
                   {
                      ...
                      ans=min(ans,(end-len+1));
                      ust.erase(str.at(len));
                      len=end-1;
                      flag=true;
